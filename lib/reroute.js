...
const log$1 = debug__default['default']('reroute:log');
log$1.log = console.log.bind(console);
...
const rerouteMiddleware = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* (opts = {}, handler, next) {
    ...
    try {
      ...
      const keyExists = yield doesKeyExist(request.uri, log$1); // Detect if needing friendly URLs
      ...
      if (isUnFriendlyUrl && (!keyExists || isIndex)) {
        ...
        if (match) {
          ...
          log$1('NO Match'); // Pass-Through: No match, so other then DefaultDoc, let it pass through
      ...
      log$1('Throwing Error for main thread');
      throw err;
    }

    log$1('RETURNING EVENT!!!!', handler.event);
    return;
  });
...
const parseRules = stringFile => {
  log$1('Parsing String: ', stringFile);
...
const doesKeyExist = rawKey => {
  ...
  if (rulesURL == "") {
    ...
    return cache.get(cacheKey, () => axios__default['default'].get(rulesURL).then(data => {
...
const getRedirectData = () => {
  ...
  if (rulesURL == "") {
    ...
    rulesURL = rulesURL.slice(-1) == "/" ? rulesURL.slice(0, -1) + Key : rulesURL + Key;
    return cache.get(cacheKey, () => {
      log(`
        Getting Rules from: ${rulesURL}
        Key: ${Key}`);
      return axios__default['default'].get(rulesURL).then(response => {
        parseRules(response.data.toString());
      }).catch(err => {
        log('No _redirects file', err);
        return false;
      });
...
const getProxyResponse = resp => {
  const status = resp.status,
        ...
        data = resp.data;
  log$1('getProxyResponse raw headers: ', resp.headers);
  ...
  log$1('getProxyResponse parse headers: ', headers);
  const response = {
    status,
    statusDescription,
    headers,
...
const get404Response = () => {
  ...
  if (rulesURL == "") {
    ...
    rulesURL = rulesURL.slice(-1) == "/" ? rulesURL.slice(0, -1) + Key : rulesURL + Key;
    return cache.get(cacheKey, () => axios__default['default'].get(rulesURL).then(({
      response
    }) => {
      log('Custom 404 FOUND');
      return {
...
const rewrite = /*#__PURE__*/function () {
  var _ref4 = _asyncToGenerator(function* (to, host, event) {
    ...
    log$1('Rewriting Event: ', JSON.stringify(resp));
    return resp;
  });
...
const proxy = (url, event) => {
  ...
  log$1('PROXY config: ', config);
  return axios__default['default'](config).then(resp => {
    log$1('PROXY data: ', _omit__default['default'](resp, ['request', 'config']));
