...
const doesKeyExist = rawKey => {
  ...
  if (rulesURL == "") {
    ...
    return cache.get(cacheKey, () => axios__default['default'].default.get(rulesURL).then(data => {
  ...
  if (rulesURL == "") {
    ...
    return cache.get(cacheKey, () => {
      log(`
        ...
        Key: ${Key}`);
      rulesURL = "https://" + rulesURL + "/" + Key;
      return !!options.rules ? Promise.resolve(parseRules(options.rules)) : axios__default['default'].default.get(rulesURL).then(response => parseRules(response.data.toString())).catch(err => {
        log('No _redirects file', err);
        return false;
      });
    });
...
const get404Response = () => {
  ...
  if (rulesURL == "") {
    ...
    rulesURL = "https://" + rulesURL + "/" + Key;
    return cache.get(cacheKey, () => axios__default['default'].default.get(rulesURL).then(({
      response
    }) => {
      log('Custom 404 FOUND');
      return {
...
const redirect = (to, status) => {
  ...
  return {
    status,
    statusDescription: http$1.STATUS_CODES[status],
    headers: {
      location: [{
        key: 'Location',
        value: to
